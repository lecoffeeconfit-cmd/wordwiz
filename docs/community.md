...
- Country, Nearby, and State rankings are optional. Location is requested only after a learner chooses one of those filters; the app sends broad derived competition keys, never coordinates, a street address, or a distance. Those keys are private and are not returned by leaderboard RPCs. Global works without location.
...
- The learning competition hub has three distinct metrics: Collectors counts meaningful personal words, Retention shows remembered review accuracy, and Learning Streaks ranks the current active streak. Stats shows personal Retention and Learning Streaks teasers that open the matching full leaderboard.
- Retention requires 40 review events before ranking. Its displayed percentage is raw remembered accuracy; rank ordering uses that accuracy with a confidence curve based on review volume, so a small perfect sample cannot outrank sustained evidence. Unqualified learners see how many reviews remain.
- Learning Streaks use the same current Daily Learning Goal shown in the app. A completed quiz, game/crossword, or active flashcard review counts as one activity; the learner's own goal must be met for that date. The leaderboard uses the current active streak, while the longest streak remains a personal Stats value and does not permanently dominate the live competition. Ties use total completed goal-days, then most recent completion.
