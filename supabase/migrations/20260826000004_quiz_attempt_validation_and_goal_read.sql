-- Keep client-synced learning attempts bounded and internally consistent before
-- the XP trigger runs. This prevents a modified client from submitting a
-- score larger than its answers or an unbounded game attempt for competitive XP.
...
-- This guard also handles databases where the earlier XP function was already
-- applied before incomplete Omega attempts were excluded from that function.
create or replace function public.community_remove_incomplete_attempt_xp()
returns trigger
language plpgsql
security definer
set search_path = public
...
begin
  if exists (
    ...
    delete from public.community_xp_ledger
    where source_id = new.id
      and source_type in ('quiz_attempt', 'game_attempt');
  end if;
  return new;
end;
$$;

drop trigger if exists community_xp_quiz_z_skip_incomplete on public.quiz_attempts;
create trigger community_xp_quiz_z_skip_incomplete
after insert on public.quiz_attempts
for each row execute function public.community_remove_incomplete_attempt_xp();

-- The goal table is private. Expose only the current goal through a
-- security-definer function so another device can restore the user's setting
-- without granting table reads to the client.
create or replace function public.community_get_daily_learning_goal()
returns integer
language sql
stable
security definer
...
revoke all on function public.community_validate_quiz_attempt() from public, anon, authenticated;
revoke all on function public.community_remove_incomplete_attempt_xp() from public, anon, authenticated;
revoke all on function public.community_get_daily_learning_goal() from public, anon, authenticated;
grant execute on function public.community_get_daily_learning_goal() to authenticated;
