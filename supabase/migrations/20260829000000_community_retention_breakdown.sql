-- Add an aggregate-only retention breakdown for the public leaderboard.
-- The existing leaderboard still controls which profiles are visible; this
-- helper only returns counts for those rows (or the signed-in learner's own
-- profile when the context card asks for it).
--
-- A retention review is one answer/check, so these counts use the same unit
-- as review_count: flashcard checks, quiz answers, test answers, and game
-- answers. Mastery/Omega Tests are stored in quiz_attempts, but are separated
-- from regular quiz answers by sessionMode.
...
begin
  ...
  return (
    ...
    own_target as (
      select p.user_id, p.public_id, null::bigint as rank
